...
<html>
    ...
    <body class="nav-md">
                        ...
                        <div class="col-md-12">
                            <div class="x_panel">
                                ...
                                <div id="searchResourcesDiv" class="x_content">
                                    <div col-md-12 col-sm-12 col-xs-12 form-group top_search>
                                    ...
                                    <div class="col-sm-12">
                                        <button id="capexReptShowSummary" class="btn btn-default pull-right" type="button">View Summary</button>
                                    </div>
                                </div>
                                ...
                                <div >Toggle column: <a class="toggle-vis" data-column="0">#</a> 
                                    - <a style="color: green; background-color: #ffff42" class="toggle-vis" data-column="1" data-name='Project' 	>Project</a> 
        ...
        <script type="text/javascript">
            ...
            $('a.toggle-vis-summTbl').on('click', function(e){
                var name= $(e.target).data('name');          
                var h=$(e.target).data('hide');          
                if(!h){              
                    $('#' + name+'TimesheetsDiv').hide();
                    $(e.target).data('hide',true);
                }
                else{              
                      $('#' + name+'TimesheetsDiv').show();
                       $(e.target).data('hide',false);              
                }
            });
    ...
    //                            $('#searchResourcesDiv').hide();
            ...
            function statusAsStringstatusAsString(status) {
                var val;
                switch (status) {
                    case 0:
                        val = 'New';
            ...
            function loadDailyReports(){
                $(document).ready(function() {
                    ...
                    // for project table
                    //var projTable = $('#projectTable.projectHeaders').DataTable();
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        var projectTblHeaders = '';
                        var projectTblRows = '';
                        $('#projectHeaders').empty();
                        $('#projectContentBody').empty();
                        $('#projectTableDesc').empty();
                
                        if(data.projects.length === 0){
                            $('#duration').append('No data found...');

                            // project summary table
                            $('#projectTableDesc').append('No data found...');
                        } else{
                                $('#duration').append('Capex Report');

                                // project summary table
                                $('#projectTableDesc').append('Project Summary');
                                ...
                                projectHearders+="<tr>" +
                                    ...
                                    "<th class='TimeSpent' style=\"width: 10%\">Time Spent</th>"+
                                    "<th class='Capex' style=\"width: 10%\">Capex</th>"+ 
                                    "<th class='Opex' style=\"width: 10%\">Opex</th>"
                                projectHearders+="</tr>";

                                // project summary table
                    projectTblHeaders+=projectHearders;
                    ...
                    for(p = 0; p < data.projects.length; p++){
                        ...
                        for(d=0; d<data.projects[p].tasks.length; d++){
                            ...
                            if (role === '61' || role === '62') {
                                table1 +="<tr class='trTaskId' data-id=\"task"+data.projects[p].tasks[d].id+"\" id=\"task"+data.projects[p].tasks[d].id+"\" style=\"outline: thin solid\"><td><b><a class=\"task aTask\" href=\"#\" "+
                                        "onclick=\"showPopUp("
                            ...
                            table3+="<tr style=\"outline: thin solid\"><td class='timespent'>" + data.projects[p].tasks[d].timeSpent + "</td></tr>";
                            table4+="<tr style=\"outline: thin solid\"><td>" + data.projects[p].tasks[d].estimateHours + "</td></tr>";
                            table5+="<tr style=\"outline: thin solid\"><td>" + data.projects[p].expectedDate + "</td></tr>";
                            table6+="<tr style=\"outline: thin solid\"><td class='capex'>" + data.projects[p].tasks[d].capex + "</td></tr>";
                            table7+="<tr style=\"outline: thin solid\"><td class='opex'>" + data.projects[p].tasks[d].opex + "</td></tr>";
                            ...
                            queryTblRows += "<td class='Status tdStatus' data-status='"+statusAsString(data.projects[p].status)+"'>" + statusAsString(data.projects[p].status) + "</td>";
                        ...
                        projectTblRows += "<tr><td>" + p + "</td>";
                        projectTblRows += "<td id='Project'>" + "</td>";
                        projectTblRows += "<td id='SubProject'><b>" + "</b></td>";
                        projectTblRows += "<td id='Task'>" + "</td>";
                        projectTblRows += "<td id='Resource'>" +  "</td>";
                        projectTblRows += "<td id='Status'>" + "</td>";
                        projectTblRows += "<td id='TimeSpent'>" + "</td>";
                        projectTblRows += "<td id='Capex'>" + "</td>";
                        projectTblRows += "<td id='Opex'>" + "</td>";
                        projectTblRows += "</tr>";
                    
                    $('#timesheetHeaders').append(queryTblHeaders);
                    $('#timesheetContentBody').append(queryTblRows);  
                    
                    $('#projectHeaders').append(projectTblHeaders);
                    $('#projectContentBody').append(projectTblRows);    
                    
                    var projects=[]; var subProjects=[];var tasks=[];var resources=[];
                    
                    $('.Project').each(function(){                        
                        if($.inArray($(this).html(),projects)===-1 && $(this).html() !== 'Project')
                            projects.push($(this).html());
                    });                    
                    $('#Project').html(projects.length);
                    
                    // sub projects count
                    $('.SubProject').each(function(){                        
                        if($.inArray($(this).html(),subProjects)===-1 && $(this).html() !== 'SubProject')
                            subProjects.push($(this).html());
                    });                    
                    $('#SubProject').html(subProjects.length);
                    
                    // Tasks count
                    $('.trTaskId').each(function(){                        
                        if($.inArray($(this).data('id'),tasks)===-1)
                    ...
                    $('.Resource').each(function(){                        
                        if($.inArray($(this).html(),resources)==-1 && $(this).html() != 'Resource')
                            resources.push($(this).html());
                    });                    
                    $('#Resource').html(resources.length);
                    // status count
                    var statusObj={};                    
                    $('.tdStatus').each(function(i){
                        if(i==0){                            
                            statusObj[$(this).html()]=1;
                        }
                        else{                            
                            if(parseInt(statusObj[$(this).html()]))
                                statusObj[$(this).html()]=parseInt(statusObj[$(this).html()]) + 1;
                            else
                                statusObj[$(this).html()]=1                                
                        }
                    });                    
